            ...
            <el-form :model="form" ref="formClose" label-width="150px" class="demo-ruleForm border-b-gray">
               ...
               <!-- <el-form-item label="字段标题" class="text-right"
                ...
                <el-form-item label="验证规则" v-show="!types"
                    ...
                    <el-select v-model="form.rules" placeholder="请选择验证规则">
                ...
                <el-form-item v-show="types" class="clearfix"
                    ...
                    :label="'选项' + index"
                    :key="domain.key"
                    :prop="types? 'domains.' + index + '.value':''"
                    :rules="{required: true, message: '域名不能为空', trigger: 'blur,change'}">

                    <el-input v-model="domain.value" style="width: 80%;float:left;"></el-input>
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            this.getList();
            let self =this;
            if(this.$route.query.type == 2 || this.$route.query.type == 8 || this.$route.query.type == 9) {
                this.type = !this.type;
            }
            ...
            if(self.$route.query.type == 2 || self.$route.query.type == 8 || self.$route.query.type == 9){
                self.types = true;
            }
        },
        methods: {
                ...
                this.$refs[formClose].validate((valid) => {
                    if(valid){
                        this.$axios.post('/makesCanton/Opera/Predefined/editAttrInster',qs.stringify({
                            ...
                            com_type:self.$route.query.type,                  // 组件类型
                            ...
                            validation: self.form.rules,                // 验证规则
                            predefined_value: self.form.domains,       // 预定义内容
                            sort:self.form.sort,                      // 排序
                            remark:self.form.info,                   // 字段描述
                        })).then( (res) =>{
            ...
            getChannel(){
                ...
                this.$axios.post('/makesCanton/ConstantConfig/getConstant',qs.stringify({
                    ...
                    if(res.data.status == 100){
                        ...
                        /*self.channelList=res.data.value;
                        let channelList = self.channelList;
                        let tableData = self.tableData;

                        for (let i = 0; i < channelList.length; i++) {
                            for (let t = 0; t < tableData.length; t++) {
                                channelList[i].name.push(tableData[t].name);
                            }
                        }
